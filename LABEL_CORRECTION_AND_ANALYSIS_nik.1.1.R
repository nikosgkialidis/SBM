...
# object output
source("collpcm.undo.label.switching.1.1.R") #older version of collpcm that runs without bugs
Undone_Z_results<-collpcm.undo.label.switching.1.1(Z=Z_seq_burned,                                                      Gsamp=Knumb)
...
# Print posterior k table with latex
library(xtable)
posterior_k_table<-posterior_k
rownames(posterior_k_table)<-"Posterior P"
print.xtable(xtable(posterior_k_table),"latex",file = paste0(folder_path,"Posterior K table.txt"))
...
colnames(Tabellone)='Points'
Ordered_Tabellone<-cbind(sort(Tabellone, decreasing = TRUE))
colnames(Ordered_Tabellone)="Points"
...
for (kk in 1:K_max_seq){
  ...
  if (not_null){
    ...
    t_cluster_percentages<-merge(t(current_cluster_percentages),Tabellone,by='row.names')[-1]
    rownames(t_cluster_percentages)=colnames(O)
    ordered_cluster_percentages<-t_cluster_percentages[order(-t_cluster_percentages[,1]),]
    ordered_cluster_percentages<-as.matrix(ordered_cluster_percentages)
    summary_table<-xtable(ordered_cluster_percentages,digits = c(rep(1,ncol(ordered_cluster_percentages)), 0),
                          caption = paste0("Season ", substr(season, start = 3, stop = 4), "/",
                                           substr(season+1, start = 3, stop = 4), " for model with K = ", kk))
    # Save Latex Table
...
for (kk in 2:K_max_seq){
  ...
  barplot(ordered_stacked_table,col=coul_kk, horiz = TRUE , axes=T,las=1, border="white", cex.names=0.6)
