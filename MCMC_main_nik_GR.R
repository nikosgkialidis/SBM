...
#### Uncomment line24 if you want to run it for only one season
...
## Choose Season
#season<-2021;country<-"G1"  
season_full<-paste0(season,"-",season+1)


#Create directory inside Inference_results for the selected season
dir.create('Inference_results')
init_dir<-paste0("Inference_results//",country)
dir.create(init_dir)
dir<-paste0("Inference_results//",country,"//mcmc_Season_",season_full,country)
dir.create(dir)
...
source("READ_TABLE_RESULTS_nik_GR.R")

###   VISUALIZE AND SAVE HEATMAP FOR RESULTS TABLE
####################################################################################
library("lattice")
palf <-colorRampPalette(c("green4", "yellow2", "red2"))

season_lab = paste0(substring(season,3,4), "/",substring(season+1,3,4))

draft_names<-colnames(O)
colnames(O)<-short_names<-toupper(substr(rownames(O),1,4))

print(levelplot(t(O[nrow(O):1,]),
                col.regions=palf(100), xlab =NULL, ylab = NULL, colorkey = FALSE,
                main =  list(label=paste0("Results table season: ", season_lab), cex=2),
                scales = list(list(alternating=1), x=list(cex=1,rot=45),y=list(cex=1)),
))


# save it in folder
pdf(paste0(dir,"//Heatmap_Season_", season_full,country,".pdf"),width = 10, height=10)
...
source("LABEL_CORRECTION_AND_ANALYSIS_nik.1.1.R") #older version that runs without bugs
...
if (K_estimated>1){
  ...
  print(levelplot(t(Final_O[nrow(Final_O):1,]),
                  ...
                  panel = function(...){
                    panel.levelplot(...)
                    panel.abline(h = (length(teams)-how_many_top)+0.5, lw = 2.5)
                    panel.abline(v = how_many_top+0.5, lw =2.5)
                  }))
  
  # save it in folder
  pdf(paste0(dir,"//Heatmap_Estimated_Season_", season,country,".pdf"),width = 10, height=10)
...
playoff_teams<-rownames(Ordered_Tabellone)[1:6]

## Save workspace
save.image(paste0(dir,"//WS_Premier_Season_", season_full,country, "_", (S-burn_in_level)/1000,
